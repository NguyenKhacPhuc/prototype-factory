    ...
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {/* Done — QR code + download */}
          {isDone && (
            <div style={{ padding: 20, borderRadius: 14, background: "var(--bg-elevated)", border: "1px solid rgba(34,197,94,0.3)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M22 11.08V12a10 10 0 11-5.93-9.14" />
                  <polyline points="22 4 12 14.01 9 11.01" />
                </svg>
                <h3 style={{ fontSize: 16, fontWeight: 700, color: "#22c55e" }}>Build Complete!</h3>
              </div>

              {/* Test on Device */}
              <div style={{ padding: 16, borderRadius: 12, background: "var(--bg)", border: "1px solid var(--border)", marginBottom: 12 }}>
                <h4 style={{ fontSize: 14, fontWeight: 700, marginBottom: 10 }}>Test on Your Phone</h4>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {[
                    { step: "1", text: "Install Expo Go from App Store / Play Store" },
                    { step: "2", text: "Open Camera (iOS) or Expo Go (Android)" },
                    { step: "3", text: "Scan the QR code below" },
                  ].map(s => (
                    <div key={s.step} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <span style={{ width: 20, height: 20, borderRadius: "50%", background: "var(--accent)", color: "#fff", fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{s.step}</span>
                      <span style={{ fontSize: 13, color: "var(--text-secondary)" }}>{s.text}</span>
                    </div>
                  ))}
                </div>

                {/* QR Code placeholder — will be generated by worker */}
                {job.result?.expo_url ? (
                  <div style={{ marginTop: 16, textAlign: "center" }}>
                    <img
                      src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(job.result.expo_url)}`}
                      alt="Expo QR Code"
                      style={{ width: 200, height: 200, borderRadius: 12, border: "1px solid var(--border)" }}
                    />
                    <p style={{ fontSize: 12, fontFamily: "monospace", color: "var(--text-dim)", marginTop: 8 }}>{job.result.expo_url}</p>
                  </div>
                ) : (
                  <div style={{ marginTop: 16, padding: 16, borderRadius: 10, background: "var(--bg-elevated)", textAlign: "center" }}>
                    <p style={{ fontSize: 13, color: "var(--text-muted)" }}>To test: download the source and run</p>
                    <code style={{ fontSize: 12, color: "var(--accent)" }}>cd app && bun install && npx expo start</code>
                  </div>
                )}
              </div>
